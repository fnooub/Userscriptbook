...
(function($, window, document, undefined) {
	...
	function saveEbook() {
		var ebookTitle = $('ul.breadcrumb > li:nth-child(3)').text().trim(),
			fileName = ebookTitle,
			fileType,
			blob;
		...
		var creditsTxt = 'Truyện được tải từ ' + location.href + LINE + 'Userscript được viết bởi: Zzbaivong',
		...
		GM_xmlhttpRequest({
			...
			onload: function(response) {
				var $data = $(response.responseText),
					$chapter = $data.find('.content-chapter'),
					$next = $data.find('.chapter-next-right > a'),
					nextUrl;

				if (endDownload) return;

				chapTitle = $data.find('ul.breadcrumb > li:nth-child(5)').text().trim();
	...
	$download.insertAfter('ul.breadcrumb');

...
(function($, window, document, undefined) {
	...
	function html2text(html, noBr = false) {
		...
		if (noBr) {
			html = html.replace(/\n/g, '<br />');
			html = html.replace(/\[img\](.+?)\[\/img\]/g, '<img src="$1" /><br /><a href="$1">Click để xem ảnh</a>');
		}
		return html;
	}
		...
		GM_xmlhttpRequest({
			...
			onload: function(response) {
				...
				if (!$chapter.length) {
					...
					jepub.add(chapTitle, html2text(cleanHtml($chapter), true));
	...
	var pageName = document.title,
		...
		ebookTitle = $('h3.title').text().trim(),
		ebookAuthor = $('.info a[itemprop="author"]').text().trim(),
		ebookCover = $('.book img').attr('src'),
		ebookDesc = $('.desc-text.desc-text-full').html(),
		ebookType = [],
		beginEnd = '',
		titleError = [],
		host = location.host,

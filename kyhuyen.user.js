...
// @author       You
// @match        https://kyhuyen.com/truyen/*/*/*
...
(function($, window, document, undefined) {
	...
	function saveEbook() {
		var ebookTitle = $('title').text().trim(),
			fileName = ebookTitle,
			fileType,
			blob;
		...
		var ebookAuthor = $('.info a:eq(0)').text().trim(),
			$ebookType = $('.info a:eq(1)'),
			ebookType = [],
		...
		GM_xmlhttpRequest({
			...
			onload: function(response) {
				var $data = $(response.responseText),
					$chapter = $data.find('.story-content'),
					$next = $data.find('.contact-deploy'),
					nextUrl;

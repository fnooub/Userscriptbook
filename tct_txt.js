// ==UserScript==
// @name         TruyenCuaTui txt
// @namespace    http://tampermonkey.net/
// @version      0.1
// @description  try to take over the world!
// @author       You
// @match        https://m.truyencuatui.net/chuong/*.html
...
(function($, window, document, undefined) {
	...
	function saveEbook() {
		...
		var ebookAuthor = $('h2').text().trim(),
			$ebookType = $('p.help a'),
			ebookType = [],

			creditsTxt = LINE2 + 'Truyện được tải từ ' + location.href + LINE + 'Userscript được viết bởi: Fnooub' + LINE2,
	...
	function getContent() {
		...
		GM_xmlhttpRequest({
			method: 'GET',
			url: 'https://m.truyencuatui.net' + url,
			onload: function(response) {
				var $data = $(response.responseText),
					$chapter = $data.find('.txt'),
					$next = $data.find('.next-button'),
					nextUrl;
